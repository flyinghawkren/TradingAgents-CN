...
def get_tushare_provider() -> TushareProvider:
    ...
    如果实例未创建或当前未连接，会自动尝试连接。
    这样即使后端启动时连接失败，后续请求在Token配置好后也能自动恢复。
    """
    global _tushare_provider
    if _tushare_provider is None:
        _tushare_provider = TushareProvider()

    # 如果未连接，尝试连接（支持热修复：配置好Token后无需重启服务）
    if not _tushare_provider.connected or _tushare_provider.api is None:
        try:
            logger.info("🔄 [Tushare] 检测到未连接状态，尝试重新连接...")
            _tushare_provider.connect_sync()
        except Exception as e:
            logger.warning(f"⚠️ [Tushare] 连接失败: {e}")
